...
function isCardsMode(){
  ...
  const saved = localStorage.getItem('serverstatusLayout');
  return saved ? saved === 'cards' : true;
}
...
function renderServersView(){
  ...
  const cardsMode = isCardsMode();
  S.layoutCompact = compact;
  const panel = $('panel-servers');
  panel.classList.toggle('cards-mode', cardsMode || compact);
  normalizeServersToolbarState();
  renderOsOptions();
  if(cardsMode || compact){
...
const VIRT_PALETTE = ['#06b6d4','#8b5cf6','#16a34a','#f59e0b','#155dfc','#14b8a6','#6366f1','#a855f7','#d97706','#eab308'];
...
function usageItem(label, value, pct){
  const p = clamp(num(pct), 0, 100);
  const color = p > 90 ? 'var(--red)' : p > 70 ? 'var(--orange)' : 'var(--green)';
  return `<div class="usage-item"><span class="stat-label">${esc(label)}</span><span class="stat-value mono">${esc(value)}</span><span class="usage-bar"><i style="width:${p.toFixed(1)}%;background:${color}"></i></span></div>`;
}
...
function refreshDetail(){
  ...
  title.innerHTML = `<span class="detail-name-only">${esc(s.name || '-')}</span>`;
  ...
  const loadSeries = [
    ...
    { data: S.loadHist[s._key]?.l5 || [], color:'#16a34a', label:'load5' },
    { data: S.loadHist[s._key]?.l15 || [], color:'#f59e0b', label:'load15' }
  ];
  const latencySeries = [
    { data: S.hist[s._key]?.cu || [], color:'#155dfc', label:'联通' },
    { data: S.hist[s._key]?.ct || [], color:'#16a34a', label:'电信' },
  ...
  const card = (title, body) => `<div class="detail-card"><div class="detail-card-title">${title}</div><div class="detail-card-body">${body}</div></div>`;
  $('detailContent').innerHTML = `
    ${card('概览', `<div class="stat-row">
      ${stat('状态', `<span class="pill ${m.online ? 'on' : 'off'}">${m.online ? '在线' : '离线'}</span>`)}
      ${stat('在线时长', esc(s.uptime || '-'))}
      ${stat('虚拟化', esc(s.type || '-'))}
      ${stat('位置', esc(s.location || '-'))}
      ${stat('系统', esc(osLabel(s.os) || '-'))}
      ${stat('主机名', esc(s.host || '-'))}
    </div>`)}
    ${cpuModel ? card('处理器', `<div class="stat-row one">${stat('型号', esc(cpuModel))}</div>`) : ''}
    ${card('资源', `<div class="usage-row">
      ${usageItem('CPU', `${num(s.cpu).toFixed(1)}%`, num(s.cpu))}
      ${usageItem('内存', `${humanMinMBFromKB(s.memory_used)} / ${humanMinMBFromKB(s.memory_total)}`, m.memPct)}
      ${usageItem('硬盘', `${humanMinMBFromMB(s.hdd_used)} / ${humanMinMBFromMB(s.hdd_total)}`, m.hddPct)}
    </div>`)}
    ${card('网络', `<div class="stat-row three">
      ${stat('当前网络', `<span class="mono">${humanMinKBFromB(s.network_rx)} ↓ / ${humanMinKBFromB(s.network_tx)} ↑</span>`)}
      ${stat('总流量', `<span class="mono">${humanMinMBFromB(s.network_in)} ↓ / ${humanMinMBFromB(s.network_out)} ↑</span>`)}
      ${stat('月流量', trafficCaps(s, true))}
    </div>`)}
    ${card('连接', `<div class="stat-row">
      ${stat('TCP', `<span class="mono">${num(s.tcp_count)}</span>`)}
      ${stat('UDP', `<span class="mono">${num(s.udp_count)}</span>`)}
      ${stat('进程', `<span class="mono">${num(s.process_count)}</span>`)}
      ${stat('线程', `<span class="mono">${num(s.thread_count)}</span>`)}
      ${stat('联通|电信|移动', `<span class="mono">${packetLossLine(s)}</span>`)}
    </div>`)}
    <section class="detail-card chart-card"><div class="chart-head"><h4>负载趋势</h4>${chartLegend(loadSeries)}</div><canvas id="loadChart" class="detail-chart" height="130"></canvas></section>
    <section class="detail-card chart-card"><div class="chart-head"><h4>三网延迟</h4>${chartLegend(latencySeries)}</div><canvas id="latChart" class="detail-chart" height="150"></canvas></section>`;
...
function bindLayoutToggle(){
  ...
  btn.addEventListener('click', () => {
    const current = isCardsMode();
    localStorage.setItem('serverstatusLayout', current ? 'table' : 'cards');
    renderServersView();
  });
}
function updateLayoutToggleButton(){
  const btn = $('layoutToggle');
  if(!btn) return;
  const cards = isCardsMode();
  const tableIcon = btn.querySelector('.layout-icon-table');
  const cardsIcon = btn.querySelector('.layout-icon-cards');
  if(tableIcon) tableIcon.style.display = cards ? 'none' : 'block';
  if(cardsIcon) cardsIcon.style.display = cards ? 'block' : 'none';
  btn.title = cards ? '切换到表格' : '切换到卡片';
}
function bindTheme(){
  ...
  const isDark = saved ? saved === 'dark' : mql.matches;
  apply(isDark);
...
$('detailModal').addEventListener('click', e => { if(e.target.id === 'detailModal') closeDetail(); });
  window.addEventListener('resize', debounce(() => {
    const compact = isCompactLayout();
    const cardsMode = isCardsMode();
    if(S.layoutCompact !== compact || S.cardsMode !== cardsMode){
      S.layoutCompact = compact;
      S.cardsMode = cardsMode;
      renderActivePanel();
    }
    if(S.openDetailKey) refreshDetail();
  }, 120));
